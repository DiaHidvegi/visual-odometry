...
class Visualizer:
    def __init__(self):
        ...
        self.gs = gridspec.GridSpec(
            2, 4, figure=self.fig, height_ratios=[1, 1])
    ...
    def update(self, frame_state: FrameState, pose: np.ndarray, image):
        ...
        self.candidates_data.append(
            frame_state.cand_landmarks_image_current.shape[1])
        ...
        if not hasattr(self, 'local_trajectory_line'):
            self.local_trajectory_line, = self.local_trajectory_ax.plot(
                [], [], 'b-', label="Trajectory")
            self.local_points_scatter = self.local_trajectory_ax.scatter(
                [], [], color='green', label="Landmarks")
            self.local_trajectory_ax.set_title("Local Trajectory")
            # Add legend with fixed position
            self.local_trajectory_ax.legend(
                bbox_to_anchor=(1.0, 1.0), loc='upper right')
        self.local_trajectory_line.set_data(
            self.poses[frame_start:frame_end, 3], self.poses[frame_start:frame_end, 11])

        # Update scatter points data
        self.local_points_scatter.set_offsets(
            frame_state.landmarks_world[[0, 2], :].T)  # Transpose for scatter

        x_min, x_max, y_min, y_max = self.determine_lims(
            frame_state, frame_start, frame_end)
        ...
        if not hasattr(self, 'global_trajectory_line'):
            self.global_trajectory_line, = self.global_trajectory_ax.plot(
                [], [], 'b-', label="Trajectory")
            self.global_trajectory_ax.set_title("Global Trajectory")
        self.global_trajectory_line.set_data(
            self.poses[:, 3], self.poses[:, 11])

        # Always show the complete trajectory with padding
        x_data = self.poses[:, 3]
        y_data = self.poses[:, 11]
        x_range = max(x_data.max() - x_data.min(),
                      1e-6)  # Avoid division by zero
        y_range = max(y_data.max() - y_data.min(),
                      1e-6)  # Avoid division by zero
        ...
        self.global_trajectory_ax.set_xlim(
            x_center - max_range/2, x_center + max_range/2)
        self.global_trajectory_ax.set_ylim(
        ...
        if not hasattr(self, 'landmarks_plot'):
            self.landmarks_plot, = self.keypoints_ax.plot(
                [], [], '-', label="Landmarks", color='green')
            self.candidates_plot, = self.keypoints_ax.plot(
                [], [], '-', label="Candidates", color='red')
            self.keypoints_ax.legend()
            self.keypoints_ax.grid(True)
            self.keypoints_ax.set_xlabel('Frame')
            self.keypoints_ax.set_ylabel('Count')

        self.landmarks_plot.set_data(
            range(frame_start + 1, frame_end), self.landmarks_data[frame_start:frame_end])
        self.candidates_plot.set_data(
            range(frame_start + 1, frame_end), self.candidates_data[frame_start:frame_end])

        # Improve keypoints axis visibility
        all_counts = self.landmarks_data[frame_start:frame_end] + \
            self.candidates_data[frame_start:frame_end]
        if len(all_counts) > 0:
            # Ensure we don't get a zero max
        ...
        if not hasattr(self, 'image_display'):
            self.image_display = self.image_ax.imshow(
                image, cmap='gray', animated=False)
            self.image_ax.set_title("Current Frame")
            self.points_plot_lm, = self.image_ax.plot(
                [], [], 'go', markersize=2, label="Landmarks")
            self.points_plot_cnd, = self.image_ax.plot(
                [], [], 'rx', markersize=2, label="Candidates")
            # Add legend with fixed position
            self.image_ax.legend(bbox_to_anchor=(1.0, 1.0), loc='upper right')
        else:
            self.image_display.set_data(image)
            self.points_plot_lm.set_data(
                frame_state.landmarks_image[0, :], frame_state.landmarks_image[1, :])
            self.points_plot_cnd.set_data(
                frame_state.cand_landmarks_image_current[0, :], frame_state.cand_landmarks_image_current[1, :])
    ...
    def determine_lims(self, frame_state, frame_start, frame_end):
        ...
        x_data = np.concatenate(
            [self.poses[frame_start:frame_end, 3], frame_state.landmarks_world[0, :]])
        y_data = np.concatenate(
            [self.poses[frame_start:frame_end, 11], frame_state.landmarks_world[2, :]])
