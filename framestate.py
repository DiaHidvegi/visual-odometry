...
class FrameState:
    ...
    cand_landmarks_transform: np.ndarray     # expected shape (12 x M) or (16 x M)
    ...
    def _validate_landmarks(self):
        ...
        if self.landmarks_image.ndim != 2 or self.landmarks_image.shape[0] != 2:
            raise ValueError(f"'landmarks_image' must have dimensions 2 x K. Got shape {self.landmarks_image.shape}.")
        
        # Validate that 'landmarks_world' is 3 x K
        if self.landmarks_world.ndim != 2 or self.landmarks_world.shape[0] != 3:
            raise ValueError(f"'landmarks_world' must have dimensions 3 x K. Got shape {self.landmarks_world.shape}.")
        ...
        if self.landmarks_image.shape[1] != self.landmarks_world.shape[1]:
            raise ValueError(f"'landmarks_image' and 'landmarks_world' must have shapes (2 x K) and (3 x K). Got {self.landmarks_image.shape} and {self.landmarks_world.shape}.")
    ...
    def _validate_candidate_landmarks(self):
        ...
        if self.cand_landmarks_image_current.ndim != 2 or self.cand_landmarks_image_current.shape[0] != 2:
            raise ValueError(f"'cand_landmarks_image_current' must have dimensions 2 x M. Got shape {self.cand_landmarks_image_current.shape}.")
        ...
        if self.cand_landmarks_image_first.ndim != 2 or self.cand_landmarks_image_first.shape[0] != 2:
            raise ValueError(f"'cand_landmarks_image_first' must have dimensions 2 x M. Got shape {self.cand_landmarks_image_first.shape}.")
        ...
        if self.cand_landmarks_transform.ndim != 2 or self.cand_landmarks_transform.shape[0] not in {12, 16}:
            raise ValueError(f"'cand_landmarks_transform' must have dimensions (12 x M) or (16 x M). Got shape {self.cand_landmarks_transform.shape}.")
        ...
        if num_cand_landmarks_current != num_cand_landmarks_first or \
           num_cand_landmarks_current != num_cand_landmarks_transform:
            raise ValueError(f"'cand_landmarks_image_current', 'cand_landmarks_image_first', and 'cand_landmarks_transform' must all refer to the same number of landmarks (M). Got shapes {self.cand_landmarks_image_current.shape}, {self.cand_landmarks_image_first.shape}, and {self.cand_landmarks_transform.shape}.")
