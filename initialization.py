...
class Initialization:
    def __init__(self, dataset, show_plots=False):
        ...
        self.points3D, self.points2D = self.get_features_and_k(self.imgs, K, params)
    ...
    def get_features_and_k(self, imgs, K, params):
        ...
        error1 = np.linalg.norm(good_old.squeeze(axis=1).T - reprojected1[:2], axis=0)
        error2 = np.linalg.norm(good_new.squeeze(axis=1).T - reprojected2[:2], axis=0)

        # Set threshold to flag outliers
        threshold = params["repro_threshold"]  # in pixels
        outliers = np.where((error1 > threshold) | (error2 > threshold) | (points3D[2]  < 0))
    ...
    def make_plots(self):
        ...
        for pt_old, pt_new in zip(self._plt_good_old, self._plt_good_new):
            ...
            cv2.circle(img_tracked, (int(x_new), int(y_new)), 3, (255, 255, 125), -1)
            
            # Draw a line from the old position to the new position
            cv2.line(img_tracked, (int(x_old), int(y_old)), (int(x_new), int(y_new)), (0, 255, 0), 1)
        ...
        ax.scatter(self._plt_points3D[0], self._plt_points3D[2], c='blue', marker='o', alpha=0.7, label='Landmarks')
