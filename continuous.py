...
class ContinuousVO:
    ...
    def _compute_baseline_angle(self, point3D: np.ndarray, t_cur: np.ndarray, t_first: np.ndarray) -> float:
        ...
        v_cur = v_cur / np.linalg.norm(v_cur).astype(np.float32)
        v_first = v_first / np.linalg.norm(v_first).astype(np.float32)

        # Compute angle between vectors
        cos_angle = np.clip(np.dot(v_cur, v_first), -
                            1.0, 1.0).astype(np.float32)
        angle = np.arccos(cos_angle).astype(np.float32)
    ...
    def _calculate_reprojection_error(self, points3D: np.ndarray, points2D: np.ndarray,
        ...
        error = np.mean(np.linalg.norm(
            points2D - projected_points, axis=1)).astype(np.float32)
        return error
    ...
    def _handle_candidates(self, img_current: np.ndarray, img_prev: np.ndarray,
        ...
        distances = np.sqrt(np.sum((Ci - Fi) ** 2, axis=0)).astype(np.float32)
        ...
        min_distances = np.min(distance_matrix, axis=1).astype(np.float32)
